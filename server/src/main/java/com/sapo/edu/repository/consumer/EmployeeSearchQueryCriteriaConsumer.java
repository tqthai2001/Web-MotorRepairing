...
public class EmployeeSearchQueryCriteriaConsumer implements Consumer<SearchCriteria> {
    ...
    public void accept(SearchCriteria param) {
        if (param.getOperation().equalsIgnoreCase(">=")) {
            ...
            if (param.getKey()
                    .equalsIgnoreCase(ConstKeywords.KEYWORD_SEARCH)) { // search keyword by: name, phone, code, address
                ...
                keywordFilter = criteriaBuilder.or(keywordFilter, criteriaBuilder.like(root.<String>get("phone"), "%" + param.getValue() + "%"));
                keywordFilter = criteriaBuilder.or(keywordFilter, criteriaBuilder.like(root.<String>get("address"), "%" + param.getValue() + "%"));
                predicate = criteriaBuilder.and(predicate, keywordFilter);
            } else if (root.get(param.getKey()).getJavaType() == Boolean.class) {
                predicate = criteriaBuilder.and(predicate, criteriaBuilder.equal(root.get(param.getKey()), Boolean.valueOf(param.getValue()
                        .toString())));
            ...
            Matcher matcher = pattern.matcher(param.getValue()
                    .toString() + ","); // add "," for the last item to be matched
