...
public class TicketSearchQueryCriteriaConsumer implements Consumer<SearchCriteria> {
    ...
    public void accept(SearchCriteria param) {
        if (param.getOperation().equalsIgnoreCase(">=")) {
            predicate = criteriaBuilder.and(predicate, criteriaBuilder.greaterThanOrEqualTo(root.get("updatedDate"), DateConverter.toLocalDateTime(param.getValue()
                    .toString())));
        } else if (param.getOperation().equalsIgnoreCase("<=")) {
            predicate = criteriaBuilder.and(predicate, criteriaBuilder.lessThanOrEqualTo(root.get("updatedDate"), DateConverter.toLocalDateTime(param.getValue()
                    .toString()).plusDays(1)));
        } else if (param.getOperation().equalsIgnoreCase("==")) {
            switch (param.getKey()) {
                case ConstKeywords.KEYWORD_SEARCH:
                    ...
                    keywordFilter = criteriaBuilder.or(keywordFilter, criteriaBuilder.like(root.<Motorbike>get("motorbike")
                            .<String>get("licensePlates"), "%" + param.getValue() + "%")); // keyword in licensePlates of motorbike
                    keywordFilter = criteriaBuilder.or(keywordFilter, criteriaBuilder.like(join.<String>get("phone"), "%" + param.getValue() + "%")); // keyword in phone of customer
                ...
                case "motorbike":
                    predicate = criteriaBuilder.and(predicate, criteriaBuilder.like(root.<Motorbike>get(param.getKey())
                            .<String>get("licensePlates"), "%" + param.getValue() + "%"));
                    break;
                case "status":
                    Byte reqStatus = Byte.valueOf(param.getValue().toString());
                    predicate = criteriaBuilder.and(predicate, criteriaBuilder.equal(root.get(param.getKey()), reqStatus));
                default:
                    break;
            }
        }
